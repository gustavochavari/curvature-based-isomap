...
def main():
    ...
    #Create 3D cube
    x1 = np.linspace(-5, 5, 11)  # 11 pontos de -5 a 5
    y1 = np.linspace(-5, 5, 11)
    z1 = np.linspace(-5, 5, 11)
    X1, Y1, Z1 = np.meshgrid(x1, y1, z1)
    
    X_Cube = np.array([X1.flatten(), Y1.flatten(), Z1.flatten()]).T.astype(np.float64)

    # Map each leaf in a cluster
    leafs = np.array([cm.rainbow(valor) for valor in X_Cube.T[2]/10+0.5])
    unique_rows, _ = np.unique(leafs.reshape(-1, leafs.shape[-1]), axis=0, return_inverse=True)
    row_to_int = {tuple(row): idx for idx, row in enumerate(unique_rows)}
    X_Cube_Labels = np.array([row_to_int[tuple(row)] for row in leafs.reshape(-1, leafs.shape[-1])]).reshape(leafs.shape[:-1])
    ...
    data_matrix = np.column_stack((x.flatten(), y.flatten(), z.flatten(), np.full(len(z.flatten()),0.2)))
    data_matrix_1 = np.column_stack((x_1.flatten(), y_1.flatten(), z_1.flatten(),np.full(len(z_1.flatten()),0.8)))

    double_torus = np.vstack([data_matrix, data_matrix_1]).astype('float64')
    X_Torus = double_torus[:,:3]
    X_Torus_Labels = double_torus.T[3]
    ...
    # File result
    file_results = 'results_all_datasets.json'
    results = {}
    ...
    for dataset in datasets:
        ...
        for r in range(len(magnitude)):
            ...
            ri, ch, fm, v = [], [], [], []
            for i in range(11):
                DR_method = 'K-ISOMAP ' + dataset_name + ' option=' + str(i) + ' cluster=' + CLUSTER
            ...
            print('Best metric is ', ri_best_metric)
            ...
            print('Best metric is ', ch_best_metric)
    ...
    if plot_results:
        ...
        print('----------------------------------------------')
        ri_star = max(enumerate(ri), key=lambda x: x[1])[0]
        print('Best option metric is ', ri_star)
        ...
        PlotaDados(dados_kiso, labels_kiso, dataset_name + ' K-ISOMAP RI option = '+ str(ri_star) + ' - K = ' + str(nn))


        # Find best result in terms of Rand index
        print('Best K-ISOMAP result in terms of Calinski-Harabasz')
        print('-----------------------------------------------------')
        ch_star = max(enumerate(ch), key=lambda x: x[1])[0]
        print('Best option metric is ', ch_star)
        dados_kiso = KIsomap(dataset_data, nn, 2, ch_star) 
        L_kiso = Clustering(dados_kiso.T, dataset_target, 'K-ISOMAP', CLUSTER)
        labels_kiso = L_kiso[4]
        PlotaDados(dados_kiso, labels_kiso, dataset_name + ' K-ISOMAP CH option = '+ str(ch_star) + ' - K = ' + str(nn))


        # Find best result in terms of Fowlkes Mallows
        print('Best K-ISOMAP result in terms of Fowlkes Mallows')
        print('----------------------------------------------')
        fm_star = max(enumerate(fm), key=lambda x: x[1])[0]
        print('Best option metric is ', fm_star)
        dados_kiso = KIsomap(dataset_data, nn, 2, fm_star) 
        L_kiso = Clustering(dados_kiso.T, dataset_target, 'K-ISOMAP', CLUSTER)
        labels_kiso = L_kiso[4]
        PlotaDados(dados_kiso, labels_kiso, dataset_name + dataset_name + ' K-ISOMAP FM option = '+ str(fm_star) + ' - K = ' + str(nn))


        # Find best result in terms of V measure
        print('Best K-ISOMAP result in terms of V measure')
        print('-----------------------------------------------------')
        v_star = max(enumerate(v), key=lambda x: x[1])[0]
        print('Best option metric is ', v_star)
        dados_kiso = KIsomap(dataset_data, nn, 2, v_star) 
        L_kiso = Clustering(dados_kiso.T, dataset_target, 'K-ISOMAP', CLUSTER)
        labels_kiso = L_kiso[4]
        PlotaDados(dados_kiso, labels_kiso, dataset_name + ' K-ISOMAP VS option = ' + str(v_star) + ' - K = ' + str(nn))
        ...
        labels_iso = L_iso[4]
        PlotaDados(isomap_data.T, labels_iso, dataset_name + ' ISOMAP ' + ' K = ' + str(nn))
        ...
        PlotaDados(umap_data.T, labels_umap, dataset_name + ' UMAP ' + ' K = ' + str(nn))
