...
def CompareKIsomap(X,color,dataset_name,k):
    ...
    ax.scatter(X.T[0], X.T[1], -10*np.ones_like(X.T[2]),c='gray',alpha=0.005)
    ...
    plt.savefig(dataset_name+'_kiso_vs_iso.jpeg',format='jpeg', dpi=300)
    plt.close()
...
def main():
    ...
    X1, y1 = RepliclustArchetype()
    X2, y2 = SwissRoll()
    X3, y3 = HopfLink(noise=True)

    k1 = int(np.floor(np.sqrt(X1.shape[0])))
    k2 = int(np.floor(np.sqrt(X2.shape[0]))) - 20
    k3 = int(np.floor(np.sqrt(X3.shape[0])))
    

    CompareKIsomap(X1,y1,'Repliclust Archetype',k1)
    CompareKIsomap(X2,y2,'Swiss Roll',k2)
    CompareKIsomap(X3,y3,'Hopf Link',k3)
